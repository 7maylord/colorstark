...
fn test_start_game() {
    ...
    // Verify bottles and targets are different (shuffled)
    let mut bottles_same = true;
    let mut i = 0;
    while i < 5 {
        if *bottles.at(i) != *target.at(i) {
            bottles_same = false;
            break;
        }
        i += 1;
...
fn test_get_correct_bottles() {
    ...
    assert!(correct_after_move <= 5, "Correct count after move too high");
